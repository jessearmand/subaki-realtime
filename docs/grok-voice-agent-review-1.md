...
**Date**: 2026-05 (exploration) · **Updated**: 2026-06-03 (implementation status) · **Docs refresh**: 2026-08-10 (language + stale facts)  
...
**Scope**: Review of `lib/realtime/xai-agent.ts` + persona-driven Grok voice config, cross-referenced against official xAI docs (voice overview + custom-voices) and local xAI Cookbook (`~/Develop/xai-cookbook/voice-examples/` + related).

## Implementation status (2026-06-03 · refreshed 2026-08-10)
...
| 4 | Custom voices (product + console) | **Partial** — `XaiVoice` widened to full built-in roster + `(string & {})` for custom IDs; runtime already passes `agent.voice`. Create/list UI and per-persona clone workflow still roadmap |
...
| — | Multilingual / Japanese S2S | **Docs only (2026-08-10)** — no API `language` on realtime; pin via `instructions` + `firstMessage`. See [xai-voice-agent-api.md](./xai-voice-agent-api.md#multilingual--japanese-s2s) |
...
- **Persona personality lives in `lib/realtime/xai-agent.ts`** (the single source of truth, analogous to ElevenLabs agent configs): `model`, built-in `voice`, `instructions` (system prompt), `firstMessage` (bootstrap via `conversation.item.create`), `tools`, `turnDetection`. **No `language` field** — S2S language is instruction-driven (see Multilingual note below and [xai-voice-agent-api.md](./xai-voice-agent-api.md)).
- `resolveXaiAgent(personaId)` merges per-persona overrides over a small `BASE`.
- 7 personas (aria/onyx/sage/nova/echo/cipher/vesper) map onto the expanded Grok built-in roster (not only the original five) with hand-crafted spoken guardrails + character prompts.
...
- https://docs.x.ai/developers/model-capabilities/audio/voice.md (Voice overview — current home for S2S; the old `…/voice-agent` path 404s as of 2026-08)
- https://docs.x.ai/developers/model-capabilities/audio/text-to-speech.md · speech-to-text.md (standalone APIs; these **do** take `language`)
...
- Supporting pages (ephemeral-tokens, models, full realtime reference)
...
- **Typing done** — `XaiVoice` is the expanded built-in roster plus `(string & {})`, so custom IDs type-check; runtime already passes `agent.voice` through.
- **Product still open** — no create/list UI, no per-persona clone workflow. Some personas still use nearest-built-in timbre matches rather than dedicated clones.
- Mock transcript in `lib/data.ts` still talks about "clone your own from a 30-second sample" — UI promise not fully backed by an xAI admin path in-app.
...
- Metadata on create: `tone`, `accent`, `use_case` ("conversational"), `gender`, `language` (e.g. `ja` for a Japanese reference clip), etc. — helps quality. Clone `language` metadata does **not** replace S2S instruction-based language control.
...
**Files (future)**: `xai-agent.ts` (per-persona ids), new `lib/realtime/xai-voices.ts` or similar?, token + any admin routes, UI picker, docs/README updates.

### 4b. Multilingual / Japanese (S2S) — docs note, 2026-08-10

**No dedicated `language` parameter on the realtime voice-agent session.** Published `session.update` examples only cover `voice`, `instructions`, `turn_detection`, `tools`, and audio formats. Standalone TTS requires `language` (JP = `ja`); STT accepts optional `language` for formatting. Those fields do **not** configure S2S.

- Default: model auto-detects spoken language and usually answers in kind (JP supported on the voice stack).
- To pin Japanese: put it in `instructions` and align `firstMessage` (Tsubaki: `lib/realtime/xai-agent.ts`).
- Example instruction line: `Always converse in Japanese (日本語). Respond only in natural Japanese unless the user explicitly asks for another language.`
- Example firstMessage: `日本語で、短く自己紹介してから用件を聞いて。`
- Unverified optional: some secondary writeups mention `audio.input.transcription.language_hint` (BCP-47). Not in the Voice overview examples; not sent by Tsubaki; would only bias input transcription if real.

Authoritative local write-up: [xai-voice-agent-api.md — Multilingual / Japanese (S2S)](./xai-voice-agent-api.md#multilingual--japanese-s2s).
...
- ~~**Type for voice**~~: **Done** — expanded roster + `(string & {})` for custom IDs.
...
**Docs (authoritative)**:
- [Voice overview](https://docs.x.ai/developers/model-capabilities/audio/voice.md) (S2S entry point; old `…/voice-agent` URL 404s) — session shape, tools, demos. **Language for S2S = instructions, not a session `language` field.**
- [Text to Speech](https://docs.x.ai/developers/model-capabilities/audio/text-to-speech.md) — required `language` (BCP-47 / `auto`; JP = `ja`).
- [Speech to Text](https://docs.x.ai/developers/model-capabilities/audio/speech-to-text.md) — optional `language` for formatting; JP supported.
- Custom Voices (creation, recording guidelines, metadata fields including `language`, voice_id interchangeability, limits, geo).
- Ephemeral Tokens (exact browser subprotocol).
- Models + Pricing (text input billing, rate limits).
- Local snapshot + JP recipe: [xai-voice-agent-api.md](./xai-voice-agent-api.md).
...
- Switch provider to xAI (Grok) for each of the 7 personas.
...
3. ~~Voice typing widened for custom IDs~~ — done (create/list UI still open)
4. Model pinning + prompt compression pass (optional)
5. Custom voice create/list + per-persona clones (roadmap)
6. JP / multilingual: prompt-only today; no code change required unless product wants a first-class language toggle

---

**Final note**: Original exploration (2026-05) recommended no immediate changes. Follow-up implementation (2026-05-30) addressed the two highest-impact items only. **2026-08-10 docs refresh** corrected stale voice/persona counts, upstream URL drift, custom-voice typing status, and documented S2S multilingual setup (instructions, not API `language`). This file is the authoritative in-repo copy of that review plus current status.
